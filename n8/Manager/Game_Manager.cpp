...
Game_Manager::~Game_Manager(){
    ...
    for (ii= registered_entities_.begin(); ii != registered_entities_.end(); ii++) {
        delete ii->second;
    }
...
System* Game_Manager::create_system(string ID){
    if (ID == BASE_SYSTEM) {
        ...
        Render_System* newSystem = new Render_System();
        newSystem->connect_message_handler_(message_handler_);
        if( add_system(ID, newSystem) ){
            return newSystem;
        }
        ...
        Movement_System* newSystem = new Movement_System();
        newSystem->connect_message_handler_(message_handler_);
        if( add_system(ID, newSystem) ){
            return newSystem;
        }
        ...
        Camera_System* newSystem = new Camera_System();
        newSystem->connect_message_handler_(message_handler_);
        if( add_system(ID, newSystem) ){
            return newSystem;
        }
...
Entity* Game_Manager::create_user_entity(int id, string initName, int initX, int initY, Sprite* sprite){
    ...
    Position_Component* position = new Position_Component(POSITION, initX, initY);
...
Entity* Game_Manager::create_screen_entity(int w, int h, int bpp){
    ...
    entScreen->add_component(new Position_Component(POSITION, 0, 0));
...
Entity* Game_Manager::create_camera_entity(int x, int y, int w, int h){
    ...
    camera->add_component(new Position_Component(POSITION, x,y));
